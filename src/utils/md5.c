...
void md5(const void* input, size_t len, uint8_t* output) {
    ...
    // First process all of the full blocks.
    while (remaining >= 64) {
        _md5ProcessBlock(p, state);
        p += 64;
        remaining -= 64;
    }
    ...
    // Now process the excess. There are two possibilities. One is that the
    // remaining input is 447 bytes or less, in which case it will fit in a
    // single 64-byte block. If it's 448 or greater, then we'll need to pad
    // into an extra block, so that there's enough room to write the original
    // message size.
    memcpy(buffer, p, remaining);

    // Set the first bit after the remainder to 1.
    buffer[remaining] = 128;

    uint64_t messageLenBits = len << 3;
    if (remaining < 448) {
        memcpy(buffer + 56, &messageLenBits, sizeof(messageLenBits));
        _md5ProcessBlock(buffer, state);
    } else {
        memcpy(buffer + 120, &messageLenBits, sizeof(messageLenBits));
        _md5ProcessBlock(buffer, state);
